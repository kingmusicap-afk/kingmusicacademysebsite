...
import { InsertUser, users, enrollments, Enrollment, InsertEnrollment } from "../drizzle/schema";
...
export async function createEnrollment(data: InsertEnrollment): Promise<Enrollment | null> {
  const db = await getDb();
  if (!db) {
    console.warn("[Database] Cannot create enrollment: database not available");
    return null;
  }

  try {
    const result = await db.insert(enrollments).values(data);
    return result as unknown as Enrollment;
  } catch (error) {
...
export async function getEnrollments(): Promise<Enrollment[]> {
  const db = await getDb();
  if (!db) {
    console.warn("[Database] Cannot get enrollments: database not available");
    return [];
  }
...
export async function getEnrollmentById(id: number): Promise<Enrollment | null> {
  const db = await getDb();
  if (!db) {
    console.warn("[Database] Cannot get enrollment: database not available");
    return null;
  }
...
export async function updateEnrollmentStatus(id: number, status: string): Promise<void> {
  const db = await getDb();
  if (!db) {
